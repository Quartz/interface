...
1. An image whose size will be determined by the user’s viewport. We use the `widthRange` prop to specify the upper and lower limits of the image's potential widths, and the `size` prop to help the browser choose which is the largest image it needs.
